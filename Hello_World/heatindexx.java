...
for (int humidity=40; humidity<=100; humidity=humidity+5)
{
	...
	while(start==true)
	{
			...
			double totalfinal=(int)(-42.379+(2.04901523*user)+(10.14333127*humidityuser)-(.22475541*user*humidityuser)-(.00683783*user*user)-(.05481717*humidityuser*humidityuser)+(.00122874*user*user*humidityuser)+( .00085282*user*humidityuser*humidityuser)-(.00000199*user*user*humidityuser*humidityuser));
			System.out.print("The THI for a temperatue of "+user+" and relative of "+humidityuser+" is:"+totalfinal+"");
			if (totalfinal<=90)
			{
			System.out.println("  ADVISORY: CAUTION");
			}
			else if (totalfinal<105)
			{
				System.out.println("  ADVISORY: EXTREME CAUTION");
			}
			else if (totalfinal<126)
			{
